...
dedup = df[["Recipe_Index", "Ingredient"]].drop_duplicates()
ingredient_props = (
  dedup.groupby("Ingredient")
  .size()
  .reset_index(name = "recipe_count")
)
ingredient_props["proportion"] = (
  ingredient_props["recipe_count"] / total_recipes
)
ingredient_props = ingredient_props.sort_values(
  by = "proportion", ascending = False
)
top20_ingredients = ingredient_props.head(20)
print(top20_ingredients)
...
```{python}
rare_ingredients = ingredients_props.loc[
  ingredient_props["recipe_count"] < 20, "Ingredient"
]
...
```{r}
#ingredient prevalance top 20
library(dplyr)
library(readr)
library(ggplot2)
dr <- read_csv("choc_chip_cookie_ingredients.csv")
total_recipes <- n_distinct(df$Recipe_Index)
...
ggplot(top20, aes(x = reorder(Ingredient, proportion), y = proportion)) + geom_col() + coord_flip() + labs(
  title = "Top 20 Ingredients by Proportions of Recipes",
  x = "Ingredient", 
...
```{r}
#comparison between AllRecipes vs Epicurious vs Misc
df_recipes <- df %>%
  distinct(Recipe_Index, Rating) %>%
  mutate(
    Source = case_when(
      grepl("^AR_", Recipe_Index) ~ "AllRecipes",
      grepl("^E_", Recipe_Index) ~ "Epicurious", 
      grepl("^Misc__", Recipe_Index) ~ "Misc",
      TRUE ~ "Other"
    )
  )
ggplot(df_recipes, aes(x = Source, y = Rating)) + geom_boxplot() + 
  labs(
    title = "Rating Distribution by Recipe Source",
    x = "Source",
    y = "Rating (0-1)"
  )
...
#The first bar chart shows that there are a few ingredients that appear in nearly all recipes, while other ingredients are not as common. This shows that all cookie recipes in the database have standard ingredients. The second boxplot compares ratings across recipe sources and whether one source has higher ratings compared to another. 
